...
def optimize_portfolio(body: OptimizeRequest) -> OptimizeResponse:
    mu = np.array(body.expected_returns, dtype=float)
    cov = np.array(body.covariance, dtype=float)
    if cov.shape != (len(mu), len(mu)):
        raise ValueError("covariance must be n x n matching expected_returns")
...
def compute_risk(body: RiskRequest) -> RiskResponse:
...
@router.post("/qubo")
def quantum_qubo(body: QuboRequest) -> dict:
    mu = np.array(body.expected_returns, dtype=float)
    cov = np.array(body.covariance, dtype=float)
    opt = PortfolioOptimizer()
